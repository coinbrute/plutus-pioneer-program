...
mkPolicy oref () ctx = traceIfFalse "UTxO not consumed" hasUTxO
  ...
  where
    ...
    checkMintedAMount - case flattenValue (txInfoForge info) of
      [(cs, tn', amt)] -> cs == ownCurrencySymbol ct && tn' == tn && amt == 1
      _                -> false
...
mint = do
    ...
    case Map.keys utxos of
        ...
        oref : _ -> do
          -- primary difference here in the off chain code is not passing in the token name param in any call here
