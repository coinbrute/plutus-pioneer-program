...
mkPolicy pkh dl () ctx = traceIfFalse "TX not signed"   (txSignedBy info pkh) 
                      && traceIfFalse "Deadline missed" (to dl `contains` txInfoValidRange info)
    where
