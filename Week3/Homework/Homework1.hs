...
mkValidator dat () ctx 
    -- get if ben1 exists in the tx info signatories returns by signed by
        -- AND see if the deadline is before the range returned by inRange
    | (unPaymentPubKeyHash (beneficiary1 dat) `elem` signedBy) && (to   (deadline dat)     `contains` inRange) = true
    | (unPaymentPubKeyHash (beneficiary2 dat) `elem` signedBy) && (from (1 + deadline dat) `contains` inRange) = true
    | otherwise                                                                                                = false
    where
        -- this is the same as usual grabbing the tx info
        info :: TxInfo
        info = scriptContextTxInfo ctx

        -- we now are grabbing a list of the public key hashes from the tx info above
        signedBy :: [PubKeyHash]
        signedBy = txInfoSignatories info

        -- and we also are just grabbing a posix time range instead of a boolean like before from the tx info above
        inRange :: POSIXTimeRange
        inRange = txInfoValidRange info
