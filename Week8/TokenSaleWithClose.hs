...
module Week8.TokenSaleWithClose
    ( TokenSale (..)
    , TSRedeemer (..)
    , TSStartSchema
    , TSUseSchema
...
-- have an addition constructor in Redeemer for state Close
data TSRedeemer =
      SetPrice Integer
    | AddTokens Integer
    | BuyTokens Integer
...
-- we have made the state integers Maybe values and updated the function body 
...
-- again here our state value for integer may or may not be present so we make it a Maybe
...
-- we add in a function to take our token sale and move it to the close state running the step to move the client to Close
close :: TokenSale -> Contract w s Text ()
close ts = void $ mapErrorSM $ runStep (tsClient ts) Close

-- add the close endpoint
type TSStartSchema =
