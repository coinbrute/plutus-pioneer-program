...
import           Week8.TokenSaleWithClose          (TokenSale (..), TSStartSchema, TSUseSchema, startEndpoint, useEndpoints)
...
instance ContractModel TSModel where
    ...
    -- add the Close action to the assoiated data type
    data Action TSModel =
              Start Wallet
    ...
    -- add the close action to the action choices for the system
    ...
    -- here we take the two wallet arguments saying they must both match and then update the state
    nextState (Close v w) = do
        -- check both wallets match
        when (v == w) $ do
            -- get the state of the model
            m <- getTSState v
            case m of
                Just t -> do
                    -- deposit into w lovelace value from t and tssToken amount from t in the contract for wallet w
                    -- then update it with Nothing
                    deposit w $ lovelaceValueOf (t ^. tssLovelace)               <>
                                assetClassValue (tokens Map.! w) (t ^. tssToken)
                    (tsModel . at v) $= Nothing
                -- anything else return Unit
                _ -> return ()
            wait 1
    ...
    perform h _ cmd = case cmd of
        ...
        -- bind the endpoints to the Close action with the UseKey and associated parameters
    ...
    -- precondition Close just as the others saying it cant be used if token sale is not started
