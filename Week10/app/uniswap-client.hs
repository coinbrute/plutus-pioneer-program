...
-- console front end client for uniswap demo

main :: IO ()
main = do
    -- get number for wallet to use
    w   <- Wallet . read . head <$> getArgs
    -- get cid file for that wallet
    cid <- read                 <$> readFile (cidFile w)
    -- decode the json file for the token
    ...
    case mcs of
        ...
        Just cs -> do
            ...
            -- invoke go on the cid and cs
            go cid cs
  where
    go :: UUID -> CurrencySymbol -> IO a
    go cid cs = do
        cmd <- readCommandIO
        -- depending on command invoke the corrosponding the command
        case cmd of
            Funds                    -> getFunds cid
            Pools                    -> getPools cid
...
-- data type for use in the commands above
data Command =
      Funds
    | Pools
    | Create Integer Char Integer Char -- amount tokenA amount tokenB
...
readCommandIO = do
    -- get input and match to command returning that command else read again
...
-- convert cs and char to coin from uniswap
toCoin :: CurrencySymbol -> Char -> US.Coin c
toCoin cs tn = US.Coin $ AssetClass (cs, fromString [tn])

-- various helper functions for parameterized actions from a command
...
getFunds :: UUID -> IO ()
getFunds cid = do
    -- call endpoint funds with cid the endpoint and a unit
  ...
  where
    go = do
        ...
        e <- getStatus cid
        case e of
            -- show the funcds on the v from Funds
            Right (US.Funds v) -> showFunds v
            _                  -> go

    -- pretty prints the Value output by e
    showFunds :: Value -> IO ()
    showFunds v = do
        showCoinHeader
...
-- get the status of a contract id
...
getStatus cid = runReq defaultHttpConfig $ do
    ...
    w <- req
        ...
        NoReqBody
        -- we tell it specifically what we are sending it
